...
Hibernate가 기본 naming strategy로 `createdAt`을 `created_at`처럼 바꾸면 기존 Prisma 테이블과 맞지 않는다. 3단계에서는 명시적 `@Table`, `@Column`, `@JoinColumn`을 사용해서 모든 테이블/컬럼 이름을 정확히 적는다. Spring Boot 기본 physical naming strategy가 명시한 이름의 대소문자까지 변경하지 않도록 `PhysicalNamingStrategyStandardImpl`도 적용한다.
...
`Admin.create()`와 `@PrePersist`는 필수다. Spring bootstrap이 새 관리자를 저장할 때 not-null `createdAt`을 만족해야 하며, JPA가 null을 명시적으로 insert해 DB default를 우회하는 상황을 방지한다.
...
애플리케이션 실행 옵션에도 `-Duser.timezone=UTC`를 적용한다. API 응답은 `LocalDateTime`에 무조건 `Z`를 붙이지 않고 `ZoneOffset.UTC`로 `Instant` 변환 후 직렬화한다.
...
Flyway는 영구 제외가 아니라 Prisma 제거 전 도입한다. JPA 매핑 검증이 끝난 뒤 `docs/db-migration-ownership-spec.md`의 baseline 절차를 완료하고, 그 이후 스키마 변경의 단일 소유자로 사용한다.
