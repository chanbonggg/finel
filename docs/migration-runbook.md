...
확정 기술/보안 결정은 `docs/spring-migration-decisions.md`를 따른다.
```

## 준비 게이트

master sequence 시작 전 다음 조건을 모두 만족한다.

```text
Java 21 toolchain, Spring Boot 3.5.15, Gradle Wrapper 8.14.5 고정
backend skeleton build/test 성공
JPA Entity/Repository가 복제 개발 DB에서 validate 통과
Admin.create와 UTC createdAt 생성 검증
Product featured/category Repository 메서드 검증
운영 배포를 same-site 또는 same-origin proxy로 확정
```
...
docs/spring-migration-decisions.md: 선택지 최종 결정
docs/db-migration-ownership-spec.md: Prisma 제거 전 DB schema 소유권 전환 기준
docs/next-api-removal-spec.md: master sequence 8단계 삭제 기준
```
...
featured가 visible 제품을 limit만큼 최신순 반환
categoryId 목록이 해당 카테고리의 visible 제품만 반환
GET /api/categories without companyId가 400 반환
GET /api/sitemap-data가 숨김 제품을 제외
...
```text
공개 API 8개가 로컬에서 200/400/404 계약대로 동작한다.
운영 DB 스키마 변경이 없다.
```
...
```text
POST /api/inquiries
InquiryPersistenceService 별도 transaction
MailService 연동
...
메일 실패 후 DB 재조회에서 문의 존재
SMTP 내부 오류 상세는 공개 응답에 없음
...
DB commit 후 SMTP를 호출하며 메일 실패가 rollback을 유발하지 않는다.
...
XSRF-TOKEN 발급과 X-XSRF-TOKEN 검증
```
...
쿠키 삭제가 정상 동작한다.
Spring Security CSRF가 활성화되어 있다.
```
...
docs/db-migration-ownership-spec.md의 Flyway baseline이 개발/Testcontainers에서 검증됨
```
...
Flyway가 이후 schema 변경의 단일 소유자로 준비되어 있다.
```
...
same-site custom domain 또는 same-origin proxy 확정
```
...
public API CORS 정상
로그인/관리자 변경 요청 CSRF 정상
cross-site third-party cookie에 의존하지 않음
```
...
운영 배포가 same-site이며 CSRF 검증이 동작한다.
```
...
관리자 API가 인증 없이 성공
Next.js build 실패
Next.js lint error 발생
CSRF 없는 관리자 변경 요청이 성공
Flyway baseline/validate 실패
```
