...
참고: 전체 전환 실행 순서의 master sequence는 `docs/migration-runbook.md`를 따른다. 이 문서의 “2단계”는 Spring Boot 뼈대 생성 산출물을 가리키는 명세명이며, 이후 실행 단계 번호와 혼동하지 않는다.
...
backend/docs/public-meta-spec.md
```
...
backend/docs/auth-spec.md, product-spec.md, category-spec.md, inquiry-spec.md, mail-spec.md, public-meta-spec.md 초안이 생성되어 있다.
...
```text
아래 목록은 이 문서 작성 당시의 준비 명세 단계명이다.
전체 실행 순서의 master sequence가 아니며, 실제 실행 순서는 docs/migration-runbook.md를 따른다.
```
