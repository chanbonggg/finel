...
XSRF-TOKEN은 HttpOnly=false
상태 변경 요청에 X-XSRF-TOKEN header 포함
```
...
auth_token HttpOnly
XSRF-TOKEN 발급
로그인 POST에 X-XSRF-TOKEN header 포함
로그인 후 /admin 접근 가능
```
...
CSRF token 누락/불일치 → 403 CSRF_INVALID
rate limit 초과 → 429
```
...
```text
X-XSRF-TOKEN 누락 또는 불일치 시 403
success=false
errorCode=CSRF_INVALID
DB 변경 없음
POST /api/inquiries는 CSRF token 없이도 계약대로 동작
```
