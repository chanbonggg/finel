...
// This reader is used by the authenticated admin UI and intentionally bypasses cache.
...
export async function getCategory(id:number|string):Promise<Category|null>{const response=await apiFetch(`/api/categories/${id}`,{next:{revalidate:300,tags:['products',`category:${id}`]}});if(response.status===404)return null;return (await readJson<{category:Category}>(response)).category;}
