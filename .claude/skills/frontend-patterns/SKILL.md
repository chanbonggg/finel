...
- React 컴포넌트 빌드 (합성, props, 렌더링)
- 상태 관리 (useState, useReducer, Zustand, Context)
- 데이터 패칭 구현 (SWR, React Query, 서버 컴포넌트)
- 성능 최적화 (메모이제이션, 가상화, 코드 분할)
- 폼 작업 (유효성 검사, 제어 입력, Zod 스키마)
- 클라이언트 사이드 라우팅 및 네비게이션 처리
- 접근 가능하고 반응형 UI 패턴 빌드
...
**기억하세요**: 현대적인 프론트엔드 패턴은 유지보수 가능하고 고성능의 사용자 인터페이스를 가능하게 합니다. 프로젝트 복잡도에 맞는 패턴을 선택하세요.
