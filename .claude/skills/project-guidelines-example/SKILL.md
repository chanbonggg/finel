...
이것은 프로젝트별 스킬의 예시입니다. 본인의 프로젝트를 위한 템플릿으로 활용하세요.

실제 프로덕션 애플리케이션 기반: [Zenith](https://zenith.chat) - AI 기반 고객 발굴 플랫폼.

## 사용 시점

이 스킬이 설계된 특정 프로젝트 작업 시 참조합니다. 프로젝트 스킬에는 다음이 포함됩니다:
- 아키텍처 개요
- 파일 구조
- 코드 패턴
- 테스트 요구사항
...
2. **불변성** - 객체나 배열을 직접 변경하지 않음
3. **TDD** - 구현 전에 테스트 먼저 작성
4. **80% 커버리지** 최소 유지
5. **많은 작은 파일** - 일반적으로 200-400줄, 최대 800줄
6. 프로덕션 코드에 **console.log 사용 금지**
7. try/catch를 사용한 **적절한 에러 처리**
8. Pydantic/Zod를 사용한 **입력 유효성 검사**
