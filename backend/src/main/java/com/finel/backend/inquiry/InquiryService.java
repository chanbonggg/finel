...
@Service public class InquiryService {
    ...
    public InquiryCreateResponse create(InquiryCreateRequest request){
        ...
        catch(DataAccessException e){log.error("inquiry db write failed: type={} causeType={}", e.getClass().getName(), causeType(e));return new InquiryCreateResponse(false,"DB_WRITE_FAILED","DB_WRITE","문의 저장에 실패했습니다.",false,false,null,null);}
        try {mail.sendInquiry(saved);} catch(MailSendException e){log.warn("inquiry mail send failed: inquiryId={} type={} causeType={}", saved.getId(), e.getClass().getName(), causeType(e));return new InquiryCreateResponse(false,"MAIL_SEND_FAILED","MAIL_SEND","문의는 접수되었지만 알림 발송에 실패했습니다.",false,true,saved.getId(),null);}
